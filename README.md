...
dashboard 把 agent 設計成**你的同事**,不是「一次性問答機器」:

```
👨‍💼 專案經理(同事)
  ├─ 📂 自媒體 · 3 週衝刺規劃   ← 進行中,有空就回來繼續
  ├─ 📂 自媒體 · Q3 選題大綱
  ├─ 📂 B2B · 顧問定價策略
  └─ 📂 跨專案優先級             ← 一次性會議
```
...
| 你想做什麼 | 怎麼操作 |
|---|---|
| **延續沒講完的會議** | Sidebar 點專案經理 → 會議室列出歷次會議 → 點要繼續的那場 |
| **跟同位專家開新主題會議** | Sidebar 點專案經理 → 「+ 開新會議」→ 填主題(如「Q3 選題」)|
| **找「跟誰聊過什麼」** | 📚 歷史 → 切換「👥 按專家」視圖 → 每位 agent 一個摺疊區段 |
| **看 / 編 PM 對你的理解** | 任何對話頂部點「📝 記憶」按鈕 |
| **讓 PM 從這場對話更新對你的理解** | 「📝 記憶」面板 → 「🔬 從這場對話蒸餾」(用 Haiku,5-15 秒) |

### Tab 顯示永遠看得出「跟誰、聊什麼」

```
👨‍💼 專案經理 · 3 週衝刺規劃   ✅ 一眼看出在跟 PM 聊衝刺
🎨 品牌守護者 · Q2 視覺定調    ✅ 一眼看出在跟品牌守護者聊定調
✍️ 內容創作者 · 5/15 直播 rundown
```

不會再變成「哪個對話是專案經理?」

### 三層記憶架構(讓 agent 真的記得你)

```
┌─────────────────────────────────────────────────────────┐
│ Layer 1:工作區備忘錄(所有 agent 共用)                 │
│  ├─ 業務領域、品牌定位、客戶類型、合規要點等            │
│  └─ 一份備忘錄,所有 agent 自動拿到 ──────────────────┐ │
└──────────────────────────────────────────────────────│ │
                                                       │ │
┌──────────────────────────────────────────────────────▼─▼┐
│ Layer 2:Agent × Workspace 同事記憶(每位 agent 各有一份)│
│  ├─ PM 對你的個人理解(專案進度、偏好、決定)            │
│  ├─ 品牌守護者對你的理解(品牌規則、視覺禁忌)            │
│  └─ 點對話頂部「📝 記憶」可編輯 / 蒸餾 / 清空 ─────────┐ │
└──────────────────────────────────────────────────────│ │
                                                       │ │
┌──────────────────────────────────────────────────────▼─▼┐
│ Layer 3:當前對話訊息歷史(該場會議的對話內容)           │
└──────────────────────────────────────────────────────────┘

         三層按順序自動拼進 system prompt 注入給 agent
```

每次新對話啟動時,agent 都會自動拿到 Layer 1 + Layer 2,**不需要重新自我介紹或重問你是誰**。

### 同事記憶長什麼樣?

打開「📝 記憶」按鈕看到的內容(以 PM 為例):

```markdown
## 這位使用者是誰
- 傳產二代,本業外勞仲介 + 音響零售
- 主場 Threads 和 YouTube,目標 B2B 顧問變現

## 進行中的專案
- [自媒體 3 週衝刺] 進度:定位完成,正在規劃內容矩陣
- [Q3 選題] 尚未開始
- [B2B 定價] 規劃中
...
你可以**手動編輯**(加「請 PM 記住:X」),也可以叫 PM **從某場對話蒸餾**(用 Haiku 模型,便宜快)。

### 蒸餾規則(避免記憶失控)

1. **手動觸發**(目前版本)— 你按「🔬 從這場對話蒸餾」才跑,不會自動偷跑
2. **取代而非累積** — 蒸餾結果直接覆蓋舊記憶,避免無限膨脹
3. **長度上限 4 KB** — 超過自動截斷
4. **Workspace 隔離** — 「個人 IP」工作區的 PM 記憶 跟「外勞仲介」工作區的 PM 記憶**完全分開**,切工作區自動換腦袋
...
這樣會議室列出來一眼分得出**同專案多場會議** vs **不同專案各一場**。
